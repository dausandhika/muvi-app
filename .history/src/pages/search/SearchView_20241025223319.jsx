...
const SearchView = ({ movies, searchTerm, setSearchTerm, handleSearch }) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {movies.map(movie => (
          <Link to={`/movie/${movie.id}`} key={movie.id} className="block">
            <div className={`rounded overflow-hidden shadow-lg transition-transform transform hover:scale-105 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
              <img
